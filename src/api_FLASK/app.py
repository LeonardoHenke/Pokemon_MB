...
def organiza_resposta(status, dict_escolha_pokemon, i, dados=None):
    ...
    return make_response(
        jsonify({'Status': status, 'StatusMessage': mensagem, 'Data': dados}),
        status,
    )
...
def consultar_pokemon():
    ...
    for i in range(1, 3):
        ...
        if (
            not str(dict_escolha_pokemon[f'pokemon{i}']).isdigit()
            or not 1 <= int(dict_escolha_pokemon[f'pokemon{i}']) <= 1025
        ):
        ...
        try:
            ...
            response_json.append(
                {'id': id_pokemon, 'nome': nome, 'tipo': tipo}
            )
...
def batalhar():
    ...
    if (
        tipo_pokemon2
        in lista_vantagens_desvantagens[tipo_pokemon1]['strong_against']
    ):
        vantagem1 = True
        
    if (
        tipo_pokemon1
        in lista_vantagens_desvantagens[tipo_pokemon2]['strong_against']
    ):
        vantagem2 = True
    ...
    if vencedor_nome is not None:
        resultado = [
            f'{vencedor_tipo.capitalize()} tem vantagem sobre {perdedor_tipo.capitalize()}. {vencedor_nome} vence!'
        ]
    else:
